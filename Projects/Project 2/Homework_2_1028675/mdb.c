...
long find_correct_instruction(BREAKPOINT breakpoint)
{
...
void serve_breakpoint(Elf *elf, int pid)
{
    ...
    long correct_instruction = find_correct_instruction(*brkpoint); // brkpoint->instruction; // find_correct_instruction(brkpoint->address);
...
// Unset the breakpoint (when deleted)
void unset_breakpoint(int pid, BREAKPOINT breakpoint)
{
    long correct_instruction = find_correct_instruction(breakpoint);
    if (ptrace(PTRACE_POKEDATA, pid, (void *)breakpoint.address, (void *)correct_instruction) == -1)
        DIE("(pokedata) %s", strerror(errno));
}
...
bool remove_breakpoint(int pid, int breakpoint_no, bool process_started)
{
    ...
    if (process_started)
    {
        unset_breakpoint(pid, breakpoint);
    }
...
int run_gdb(char **argv)
{
    ...
    while (true)
    {
        ...
        else if (strcmp(command, "disas") != 0 && strncmp(command, "d", strlen("d")) == 0)
        {
            ...
            else if (!remove_breakpoint(pid, breakpoint_no, process_started))
            {
